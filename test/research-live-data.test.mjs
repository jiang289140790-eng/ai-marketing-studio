...
test('开发用本地示例回退：显式构建、明确标注、默认由页面关闭', () => {
  ...
  assert.ok(pageSource.includes('useState(false)'), '页面必须默认关闭开发回退');
  assert.ok(pageSource.includes('devFallbackOn'), '页面必须由显式开关控制开发回退');
  assert.ok(pageSource.includes('!configured && devFallbackOn'), '开发回退只能在未配置运行时被激活');
});
