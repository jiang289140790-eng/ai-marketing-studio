...
线上界面与数据层已完成；Bridge 与 MCP 已部署到 AutoDL 内网，但尚未取得不影响现有服务的公网 HTTPS 入口。

本次没有配置假的 Bridge 地址，没有创建临时公网地址充当生产域名，也没有向 Supabase 写入 `OPS_MCP_BRIDGE_URL` 或 `OPS_MCP_BRIDGE_SECRET`。

## 当前阻塞条件

缺少：**可供 Bridge 独占或反向代理的公网 HTTPS 入口**。
...
| AutoDL / SeetaCloud | 2026-07-23 已恢复，SSH 与外网均可用；公网映射端口 6006、6008 均被现有服务占用 | Bridge 已在内网运行，公网入口仍阻塞 |
...
AutoDL 实例恢复后，已在独立目录部署 Bridge 与 AI Marketing Studio MCP，不修改现有 ComfyUI 与控制面板：
...
| MCP 单元测试 | 7/7 通过 |
| Bridge 安装与 lint | 通过 |
| Bridge 内网监听 | `127.0.0.1:8787` |
| 正确 HMAC 健康检查 | HTTP 200 |
| `bridge` / `mcp` | `true` / `true` |
| MCP 工具数量 | 50（health 返回上限） |
| X MCP | 未接入，准确显示 `unknown` |
| ComfyUI | 已配置为本机 `127.0.0.1:6006` |
| 真实发布 | 关闭，`ALLOW_REAL_PUBLISH=false` |

AutoDL 当前端口占用：

- `6006`：ComfyUI；
- `6008`：现有 ComfyUI Control Panel；
- `8787`：AI Marketing Studio Bridge，仅内网监听。

AutoDL 提供的两个稳定公网 HTTPS 映射分别对应 6006 与 6008。直接将 Bridge 绑定到任一端口都会中断现有功能；现有控制面板还将 6008 写死，不能通过普通环境变量迁移。因此在取得“改造 6008 为反向代理”授权或新增稳定公网入口之前，不能把 Bridge URL 写入 Supabase。

本次还修复了健康检查中将字符串 `"false"` 误判为 X MCP 已配置的问题，避免线上出现假绿灯。
