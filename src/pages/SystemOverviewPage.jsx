...
export function SystemOverviewPage({ userId }) {
  const [data, setData] = useState({ agentRuns: [], workflowRuns: [], publishTasks: [], contentMetrics: [] });

  useEffect(() => {
    if (!userId || !isSupabaseConfigured) return undefined;
    loadSystemStatusData().then((nextData) => setData({ agentRuns: [], workflowRuns: [], publishTasks: [], contentMetrics: [], ...nextData }));
    return undefined;
  }, [userId]);
  ...
  return (
    <section className="page-stack">
      <div className="hero-panel">
        ...
        <h2>只看运营是否稳定，不展示底层数据库细节</h2>
        <p>这里汇总 Agent、Workflow、发布任务和指标回收的状态，帮助你发现失败任务和需要重试的环节。</p>
      </div>
