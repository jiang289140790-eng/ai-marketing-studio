...
async function reserve(db, userId: string, provider: string, amount: number) {
  ...
  if (error) {
    if (/P22_DAILY_BUDGET_EXCEEDED/.test(error.message)) throw new P22Error('DAILY_BUDGET_EXCEEDED',`${provider} 今日 ¥10 预算已不足。`,429);
    throw new P22Error('BUDGET_RESERVATION_FAILED','无法安全预留调用预算。',503);
  }
  return data;
}

async function budgetStatus(db) {
  ...
  if(error) throw new P22Error('BUDGET_STATUS_UNAVAILABLE','无法安全读取今日预算状态。',503);
...
async function analyze(db,userId:string,items,proofSecret:string) {
  ...
  const budget=await reserve(db,userId,'qwen',P22_LIMITS.qwen_reservation_cny);
  ...
  try {
    ...
    return {analyses:parseQwenAnalyses(payload,items),usage:{total_tokens:totalTokens},cost:{reserved_cny:P22_LIMITS.qwen_reservation_cny,budget}};
